...
(function() {
    ...
    this.attach = function(callback) {
        ...
        if (this._connected) throw new Error("IKPdbService already attached!");
        ...
        this._socket.on("end", function() {
            for (var id in self._callbacks) {
                if (!self._callbacks[id]) continue;
    ...
    this._receiveMessage = function(message) {
        ...
        if (content.command == "programBreak") return this._breakHandler(content);

        // run pending callback if sequence number matches one we sent
        if (typeof content._id == "undefined") return;
        // execute callback
        var callback = null;
