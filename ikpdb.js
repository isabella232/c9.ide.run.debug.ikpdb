...
main.consumes = ["Plugin", "c9", "util", "debugger", "dialog.error", "notification.bubble"];
main.provides = ["debugger.ikpdb"];
module.exports = main;
...
function main(options, imports, register) {
    ...
    function sendCommand(command, args, callback) {
        ikpdbs.sendCommand(command, args, function(err, reply) {
            ...
            if (reply.result.executionStatus && reply.command !== "runScript")
    ...
    /**
     * Applies a series of user defined filters to a message received from IKPdbService
     */
    function applyMessageFilters(message) {
        if (!messageFilters || !messageFilters.length) return message;

        messageFilters.forEach(function(filter) {
    ...
    function programBreak(message) {
        ...
        if (message.exception) {
            showError(
                'IKpdb has detected an unmanaged exception "' +
            ...
            if (stack.length == 1) debug.getElement("btnStepOut").setAttribute("disabled", true);
        }
    }
    ...
    function breakpointManagementWarning(commandSpecificMessage) {
        ...
        if (state == "running") {
            var msg =
                "IKPdpb Error: You can't manage breakpoints while" +
    ...
    function setBreakpoint(bp, callback) {
        ...
        breakpointManagementWarning(
            "Breakpoint creation request will be send to debugger at next programm break !."
        );

        if (pathMap) {
            var pathWithTask = applyPathMap(bp.data.path, "toExternal");
            var isPartOfProject = pathWithTask.startsWith("/var/task/");
            if (!isPartOfProject) {
                return callback && callback(null, bp, {});
            }
        }
    ...
    function changeBreakpoint(bp, callback) {
        // TODO: Check with c9 how we can prevent user to set breakpoint
        breakpointManagementWarning(
            "Breakpoint modification request will be send to debugger at next programm break !."
        );
    ...
    function clearBreakpoint(bp, callback) {
        // TODO: Check with c9 how we can prevent user to set breakpoint
        breakpointManagementWarning(
            "Breakpoint removal request will be send to debugger at next programm break !."
        );
        ...
        listBreakpoints(function(err, remoteBkpts) {
            ...
            manyBreakpoints(to_add, setBreakpoint, function(added, fail) {
                ...
                manyBreakpoints(remoteBkpts, clearBreakpoint, function(cleared, clrfail) {
                    ...
                    if (begin) runScript(callback);
                    else sendCommand("getStatus", {}, callback);
                });
            });
        });
    }
    ...
    function attach(_socket, reconnect, callback) {
        ...
        debugger_socket.on(
            "error",
            function(err) {
                if (typeof err === "object" && err.code === 0) return; // This is always thrown by ikpdb on exit
        ...
        ikpdbs.attach(function() {
            ...
            if (reconnect) reconnectSync(callback);
            else sync(true, callback);
        });
    ...
    function setPathMap(pathMappers) {
        if (!Array.isArray(pathMappers)) pathMappers = null;
        pathMap =
            pathMappers &&
            pathMappers
                .map(function(mapper) {
                    if (!mapper.toInternal || !mapper.toExternal) return;
                    var map = {
                        toInternal: {},
                        toExternal: {},
                    };
                    if (typeof mapper.toInternal.regex == "string")
                        map.toInternal.regex = new RegExp(mapper.toInternal.regex, "g");
                    map.toInternal.replacement = mapper.toInternal.replacement;
                    map.toInternal.isAbsolute = mapper.toInternal.isAbsolute;
                    if (typeof mapper.toExternal.regex == "string")
                        map.toExternal.regex = new RegExp(mapper.toExternal.regex, "g");
                    map.toExternal.replacement = mapper.toExternal.replacement;
                    map.toExternal.isAbsolute = mapper.toExternal.isAbsolute;
                    return map;
                })
                .filter(Boolean);
    }
    ...
    plugin.freezePublicAPI({
        ...
        /**
         * Sets the path map to format paths correctly for internal and external consumption
         */
        setPathMap: setPathMap,

        /**
         * Allows the user to define filters that are run over messages from the ikpdb process
         */
        setMessageFilters: setMessageFilters,
    });

    register(null, {
        "debugger.ikpdb": plugin,
    });
}
